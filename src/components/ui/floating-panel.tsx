...
interface FloatingPanelTriggerProps extends ButtonProps {
  children: React.ReactNode;
  className?: string;
  title: string;
}
...
export function FloatingPanelContent({
  ...
  const { isOpen, closeFloatingPanel, uniqueId, triggerRect } =
    useFloatingPanel();
  ...
  useLayoutEffect(() => {
    // Perbarui panelWidth setelah elemen tersedia di DOM
  ...
  useEffect(() => {
    // Memperbarui posisi panel saat jendela di-resize
    const handleResize = () => {
      setWindowWidth(window.innerWidth);
      setPanelWidth(contentRef.current?.offsetWidth ?? 0);
    };

    window.addEventListener("resize", handleResize);

    // Mengatur overflow hidden pada body saat panel terbuka
    if (isOpen) {
    ...
    return () => {
      ...
      document.body.style.overflow = ""; // Mengembalikan overflow saat panel ditutup
    };
  }, [isOpen]);
  ...
  const getPositionStyles = () => {
    ...
    // Menghindari agar tidak keluar dari jangkauan width window
    const left = Math.min(
      triggerRect.left,
      windowWidth - panelWidth - 16, // Tambahkan margin agar tidak mepet
    );
...
export function FloatingPanelTextarea({
  ...
  return (
    <textarea
      id={id}
      className={cn(
        "h-full w-full resize-none rounded-md bg-transparent text-sm outline-none",
        className,
      )}
      autoFocus
      value={note}
