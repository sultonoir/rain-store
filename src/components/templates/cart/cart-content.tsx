...
export default function CartContent() {
  const { setCartOpen, cartOpen } = useOpenCart();
  const { cart } = useCart();
  ...
  return (
    <Sheet open={cartOpen} onOpenChange={setCartOpen}>
      <SheetContent side="right" className="flex flex-col gap-0 p-0">
        <SheetHeader className="flex-shrink-0 flex-row items-center justify-between p-6">
          ...
          <div className="bg-accent dark:bg-accent/50">
            <div className="p-6">
              ...
              <Button className="w-full" asChild>
                <Link href="/checkout" onClick={setCartOpen}>
                  Checkout
                </Link>
              </Button>
            </div>
